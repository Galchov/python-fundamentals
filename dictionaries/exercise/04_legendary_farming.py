def collect_material(key_materials_dictionary: dict, junk_materials: dict, material: str, qty: int):
    if material == 'shards' or material == 'fragments' or material == 'motes':
        key_materials_dictionary[material] += qty
    else:
        if material not in junk_materials.keys():
            # Material still does not exist in the junk
            junk_materials[material] = qty
        else:
            # Material already exists in the junk and we need to add the qty
            junk_materials[material] += qty
