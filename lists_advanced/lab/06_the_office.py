...
happy_emplyoees = [el for el in happiness if el >= threshold]
...
if len(sad_employees) > len(happiness) / 2:
    print(f"Score: {len(happy_emplyoees)}/{len(happiness)}. {sad_message}!")
else:
    print(f"Score: {len(happy_emplyoees)}/{len(happiness)}. {happy_message}!")