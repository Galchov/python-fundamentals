...
def strike_at_index(collection: list, strike_index: int, radius: int):
    ...
    right_index_index = strike_index + radius

    if is_index_valid(collection, left_strike_index) and \
            is_index_valid(collection, strike_index) and \
            is_index_valid(collection, right_index_index):

        # Perform a strike
        for i in range(left_strike_index, right_index_index + 1):
            collection.pop(left_strike_index)

        # Other way to delete
        # del collection[left_strike_index:right_index_index + 1]
